...
- **Miscellaneous tab** — a salesperson's "insight lab" built from feeds the other tabs never touch: **market-basket affinity** (what rings up alongside FunPop, for cross-merchandising pitches), **hourly dayparting** (weekday × hour POS heatmap of when it sells), **tender mix** (units by tender type, weekly history + YoY where the feed carries a prior-year window — a read on shopper price-sensitivity, e.g. EBT/SNAP share), **markdowns/MUMD** (margin leakage by event/week), and **out-of-stock root cause** (the reason hierarchy behind each OOS store-day). Each section is fault-tolerant: a missing source table just shows a note instead of taking the tab down. Sources: `sql/affinity_query.sql`, `hourly_query.sql`, `tender_query.sql`, `mumd_query.sql`, `oos_query.sql`.
