...
WHERE wm_item_nbr IN UNNEST(@active_items)
  AND (store_actual_effective_date IS NULL OR store_actual_effective_date <= CURRENT_DATE('America/Chicago'))
  AND (store_actual_expiry_date IS NULL OR store_actual_expiry_date >= CURRENT_DATE('America/Chicago'))
