...
# ─── Miscellaneous / "Insight Lab" loaders ───────────────────────────────────
# These power the Miscellaneous tab from data sources the rest of the dashboard
# never touches (market-basket affinity, hourly register sales, tender mix,
# markdowns, out-of-stock root cause). Each is fully fault-tolerant: a missing
# table or empty feed returns (empty, err) and the tab shows a friendly note,
# never a crash — same contract as the other secondary loaders above.
@st.cache_data(ttl=86400, max_entries=3, show_spinner=False)
def load_affinity_data(refresh_slot: str = "") -> tuple[pd.DataFrame, str | None]:
    try:
        df = _cached_query("affinity_query.sql", [
            bigquery.ArrayQueryParameter("active_items", "INT64", ACTIVE_ITEMS),
        ])
        if not df.empty:
            _shrink_frame(df,
                int_cols=["walmart_calendar_week", "walmart_item_number",
                          "affinity_item_number", "affinity_basket_count", "total_basket_count"],
                float_cols=["affinity_item_avg_retail", "primary_item_avg_retail",
                            "avg_basket_affinity_item_count", "avg_basket_affinity_retail",
                            "avg_basket_units"])
        return df, None
    except Exception as e:
        return pd.DataFrame(), str(e)
...
@st.cache_data(ttl=86400, max_entries=3, show_spinner=False)
def load_hourly_data(lookback_days: int, refresh_slot: str = "") -> tuple[pd.DataFrame, str | None]:
    try:
        df = _cached_query("hourly_query.sql", [
            bigquery.ArrayQueryParameter("active_items", "INT64", ACTIVE_ITEMS),
            bigquery.ScalarQueryParameter("lookback_days", "INT64", lookback_days),
        ])
        if not df.empty:
            df["business_date"] = pd.to_datetime(df["business_date"])
            _shrink_frame(df,
                int_cols=["business_hour", "store_number", "walmart_item_number", "scan_count"],
                float_cols=["sales_amount"],
                cat_cols=["service_channel"])
        return df, None
    except Exception as e:
        return pd.DataFrame(), str(e)
...
@st.cache_data(ttl=86400, max_entries=3, show_spinner=False)
def load_tender_data(lookback_days: int, refresh_slot: str = "") -> tuple[pd.DataFrame, str | None]:
    try:
        df = _cached_query("tender_query.sql", [
            bigquery.ArrayQueryParameter("active_items", "INT64", ACTIVE_ITEMS),
            bigquery.ScalarQueryParameter("lookback_days", "INT64", lookback_days),
        ])
        if not df.empty:
            df["business_date"] = pd.to_datetime(df["business_date"])
            _shrink_frame(df,
                int_cols=["store_number", "walmart_item_number", "units_by_tender", "visits_by_tender"],
                float_cols=["sales_by_tender"],
                cat_cols=["period", "tender_group", "tender_type"])
        return df, None
    except Exception as e:
        return pd.DataFrame(), str(e)
...
@st.cache_data(ttl=86400, max_entries=3, show_spinner=False)
def load_mumd_data(lookback_days: int, refresh_slot: str = "") -> tuple[pd.DataFrame, str | None]:
    try:
        df = _cached_query("mumd_query.sql", [
            bigquery.ArrayQueryParameter("active_items", "INT64", ACTIVE_ITEMS),
            bigquery.ScalarQueryParameter("lookback_days", "INT64", lookback_days),
        ])
        if not df.empty:
            df["markdown_date"] = pd.to_datetime(df["markdown_date"])
            _shrink_frame(df,
                int_cols=["store_number", "walmart_item_number", "markdown_units"],
                float_cols=["markdown_amount", "previous_retail", "current_retail"],
                cat_cols=["markdown_type", "markdown_event"])
        return df, None
    except Exception as e:
        return pd.DataFrame(), str(e)
...
@st.cache_data(ttl=86400, max_entries=3, show_spinner=False)
def load_oos_data(lookback_days: int, refresh_slot: str = "") -> tuple[pd.DataFrame, str | None]:
    try:
        df = _cached_query("oos_query.sql", [
            bigquery.ArrayQueryParameter("active_items", "INT64", ACTIVE_ITEMS),
            bigquery.ScalarQueryParameter("lookback_days", "INT64", lookback_days),
        ])
        if not df.empty:
            df["business_date"] = pd.to_datetime(df["business_date"])
            _shrink_frame(df,
                int_cols=["store_number", "walmart_item_number"],
                float_cols=["fractional_instock_pct"],
                cat_cols=["oos_reason_high", "oos_reason_mid", "oos_reason_low", "replenishable_flag"])
        return df, None
    except Exception as e:
        return pd.DataFrame(), str(e)
...
# ─── Tabs ────────────────────────────────────────────────────────────────────
(tab_overview, tab_sales, tab_forecast, tab_inv, tab_channels, tab_dist,
 tab_actions, tab_misc) = st.tabs([
    "Overview",
    "Sales & Velocity",
    "Forecast",
    "Inventory & DC",
...
# ═══════════════════════════════════════════════════════════════════════════
#   TAB 8 — MISCELLANEOUS (the rep's insight lab)
# ═══════════════════════════════════════════════════════════════════════════
# Everything here is built from data sources the other tabs never touch, framed
# the way a salesperson would interrogate the account: what sells alongside us,
# when shoppers buy, how they pay, where margin leaks, and why we go dark. Each
# section loads independently and degrades to a friendly note if its feed is
# unavailable, so a single missing table never takes the tab down.
with tab_misc:
    st.subheader("Miscellaneous — the rep's insight lab")
    st.caption(
        "Less-traveled corners of the BI Link feed, read from a salesperson's chair: "
        "the basket, the clock, the wallet, the price tag, and the empty shelf. "
        "Use the sidebar item scope to focus any section."
    )

    HEAT_SCHEME = "blues"
    WEEKDAY_ORDER = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]

    # ── 1. Market basket — what sells WITH FunPop ────────────────────────────
    st.divider()
    st.subheader("🧺 Market basket — what customers buy with us")
    st.caption(
        "From item-affinity baskets: the products most often ringing up in the same trip "
        "as FunPop. Gold for a cross-merchandising pitch — 'put us next to these and lift both.' "
        "Penetration = share of FunPop baskets that also contained the partner item."
    )
    aff_df, aff_err = load_affinity_data(slot)
    if aff_err:
        _section_error("Market-basket affinity data", aff_err)
    elif aff_df.empty:
        st.info("No market-basket affinity records available.")
    else:
        aff_filt = aff_df[aff_df["walmart_item_number"].isin(item_filter)].copy()
        if aff_filt.empty:
            st.info("No affinity data for the selected items.")
        else:
            # Aggregate partner items across the recent window & selected primary
            # items: sum co-occurrence baskets, carry a representative avg retail.
            partner = aff_filt.groupby("affinity_item_name", as_index=False, observed=True).agg(
                baskets=("affinity_basket_count", "sum"),
                partner_retail=("affinity_item_avg_retail", "mean"),
            )
            total_baskets = float(aff_filt["total_basket_count"].sum())
            partner["penetration_pct"] = np.where(
                total_baskets > 0, (partner["baskets"] / total_baskets * 100).round(1), 0)
            partner = partner[partner["baskets"] > 0].sort_values(
                "baskets", ascending=False).head(15)
            partner["affinity_item_name"] = (partner["affinity_item_name"]
                                             .astype("object").fillna("(unnamed item)").astype(str))

            mb_k1, mb_k2, mb_k3 = st.columns(3)
            mb_k1.metric("Distinct partner items", f"{aff_filt['affinity_item_number'].nunique():,}")
            mb_k2.metric("Avg items in our basket",
                         f"{aff_filt['avg_basket_affinity_item_count'].mean():.1f}",
                         help="Average number of distinct items in a trip that includes FunPop.")
            mb_k3.metric("Avg basket value with us",
                         f"${aff_filt['avg_basket_affinity_retail'].mean():,.2f}",
                         help="Average retail value of a basket that includes FunPop.")

            if not partner.empty:
                st.altair_chart((alt.Chart(partner).mark_bar(color="#185FA5").encode(
                    x=alt.X("baskets:Q", title="Co-occurring baskets"),
                    y=alt.Y("affinity_item_name:N", sort="-x", title="Bought alongside FunPop"),
                    tooltip=[alt.Tooltip("affinity_item_name:N", title="Partner item"),
                             alt.Tooltip("baskets:Q", format=",", title="Baskets"),
                             alt.Tooltip("penetration_pct:Q", format=".1f", title="Basket penetration %"),
                             alt.Tooltip("partner_retail:Q", format="$,.2f", title="Partner avg retail")],
                ).properties(height=max(200, 26 * len(partner)))), width='stretch')
                top = partner.iloc[0]
                st.caption(
                    f"**Lead with this:** *{top['affinity_item_name']}* shows up in "
                    f"{top['penetration_pct']:.1f}% of FunPop baskets — the strongest natural "
                    "adjacency to argue for a shared end-cap or cross-aisle placement."
                )

    # ── 2. Dayparting — WHEN FunPop sells ────────────────────────────────────
    st.divider()
    st.subheader("🕐 Dayparting — when the register rings")
    st.caption(
        "Hourly POS scans across the lookback, by weekday and hour. Tells a rep exactly when "
        "to time a demo, a restock, or a display reset — and proves the product isn't a "
        "dead-hours impulse buy."
    )
    hr_df, hr_err = load_hourly_data(lookback, slot)
    if hr_err:
        _section_error("Hourly sales data", hr_err)
    elif hr_df.empty:
        st.info("No hourly register-sales records available.")
    else:
        hr_filt = hr_df[hr_df["walmart_item_number"].isin(item_filter)].copy()
        hr_filt = hr_filt[(hr_filt["business_hour"] >= 0) & (hr_filt["business_hour"] <= 23)]
        if hr_filt.empty:
            st.info("No hourly data for the selected items.")
        else:
            hr_filt["weekday"] = hr_filt["business_date"].dt.day_name().str[:3]
            # Hour-of-day profile
            by_hour = hr_filt.groupby("business_hour", as_index=False).agg(
                scans=("scan_count", "sum"),
                dollars=("sales_amount", "sum"),
            ).sort_values("business_hour")
            ...
            def _fmt_hour(h):
                ...
                if disp == 0:
                    disp = 12
                return f"{disp}{ap}"
            by_hour["hour_label"] = by_hour["business_hour"].map(_fmt_hour)

            peak = by_hour.loc[by_hour["scans"].idxmax()] if not by_hour.empty else None
            day_tot = hr_filt.groupby("weekday")["scan_count"].sum()
            peak_day = day_tot.idxmax() if not day_tot.empty else "—"
            d_k1, d_k2, d_k3 = st.columns(3)
            if peak is not None:
                d_k1.metric("Peak hour", _fmt_hour(peak["business_hour"]),
                            help="Hour with the most POS scans across the window.")
            d_k2.metric("Peak weekday", str(peak_day))
            d_k3.metric("Total scans (window)", f"{int(hr_filt['scan_count'].sum()):,}")

            st.altair_chart((alt.Chart(by_hour).mark_bar(color="#185FA5").encode(
                x=alt.X("hour_label:N", sort=list(by_hour["hour_label"]), title="Hour of day",
                        axis=alt.Axis(labelAngle=0)),
                y=alt.Y("scans:Q", title="POS scans"),
                tooltip=[alt.Tooltip("hour_label:N", title="Hour"),
                         alt.Tooltip("scans:Q", format=",", title="Scans"),
                         alt.Tooltip("dollars:Q", format="$,.0f", title="Sales $")],
            ).properties(height=280)), width='stretch')

            # Weekday × hour heatmap
            st.markdown("**Weekday × hour heatmap (where the demand pockets are)**")
            heat = hr_filt.groupby(["weekday", "business_hour"], as_index=False)["scan_count"].sum()
            heat["hour_label"] = heat["business_hour"].map(_fmt_hour)
            hour_order = [_fmt_hour(h) for h in range(24) if h in set(heat["business_hour"])]
            if not heat.empty:
                st.altair_chart((alt.Chart(heat).mark_rect().encode(
                    x=alt.X("hour_label:N", sort=hour_order, title="Hour", axis=alt.Axis(labelAngle=0)),
                    y=alt.Y("weekday:N", sort=WEEKDAY_ORDER, title="Weekday"),
                    color=alt.Color("scan_count:Q", scale=alt.Scale(scheme=HEAT_SCHEME),
                                    legend=alt.Legend(title="Scans")),
                    tooltip=["weekday", alt.Tooltip("hour_label:N", title="Hour"),
                             alt.Tooltip("scan_count:Q", format=",", title="Scans")],
                ).properties(height=240)), width='stretch')

    # ── 3. Tender mix — HOW shoppers pay ─────────────────────────────────────
    st.divider()
    st.subheader("💳 Tender mix — how shoppers pay")
    st.caption(
        "Units split by tender type when FunPop is in the basket. A heavy cash/EBT tilt signals "
        "a price-sensitive shopper (defend the price point, watch markdowns); a card/digital tilt "
        "signals room to trade up. Shown by week, with year-over-year where the feed carries it."
    )
    tn_df, tn_err = load_tender_data(lookback, slot)
    if tn_err:
        _section_error("Tender-mix data", tn_err)
    elif tn_df.empty:
        st.info("No tender records available.")
    else:
        tn_all = tn_df[tn_df["walmart_item_number"].isin(item_filter)].copy()
        # period tags the year-ago window (LY) vs the recent window (TY); older
        # snapshots taken before the LY window was added simply have no 'period'
        # column, so default everything to TY.
        if "period" not in tn_all.columns:
            tn_all["period"] = "TY"
        tn_all["period"] = tn_all["period"].astype("object").fillna("TY").astype(str)
        tn_ty = tn_all[tn_all["period"] == "TY"].copy()
        tn_ly = tn_all[tn_all["period"] == "LY"].copy()
        if tn_ty.empty:
            st.info("No tender data for the selected items.")
        else:
            # Per-tender rollup (units-first), with YoY units where LY exists.
            ty_t = tn_ty.groupby("tender_type", as_index=False, observed=True).agg(
                units=("units_by_tender", "sum"),
                sales=("sales_by_tender", "sum"),
                visits=("visits_by_tender", "sum"),
            )
            ly_t = (tn_ly.groupby("tender_type", as_index=False, observed=True)
                    .agg(units_ly=("units_by_tender", "sum"))
                    if not tn_ly.empty else
                    pd.DataFrame({"tender_type": [], "units_ly": []}))
            by_tender = ty_t.merge(ly_t, on="tender_type", how="left")
            by_tender["units_ly"] = by_tender["units_ly"].fillna(0)
            by_tender = by_tender[by_tender["units"] > 0].sort_values("units", ascending=False)
            by_tender["tender_type"] = (by_tender["tender_type"]
                                        .astype("object").fillna("(unspecified)").astype(str))
            tot_units = float(by_tender["units"].sum())
            by_tender["share_pct"] = np.where(
                tot_units > 0, (by_tender["units"] / tot_units * 100).round(1), 0)
            by_tender["units_per_visit"] = np.where(
                by_tender["visits"] > 0, (by_tender["units"] / by_tender["visits"]).round(2), 0)
            by_tender["yoy_pct"] = _yoy_pct(by_tender["units"], by_tender["units_ly"])
            has_ly = float(by_tender["units_ly"].sum()) > 0

            if not by_tender.empty:
                lead = by_tender.iloc[0]
                ebt_mask = by_tender["tender_type"].str.contains("EBT|SNAP|FOOD", case=False, na=False)
                ebt_share = float(by_tender.loc[ebt_mask, "share_pct"].sum())
                t_k1, t_k2, t_k3 = st.columns(3)
                t_k1.metric("Leading tender (units)", str(lead["tender_type"]),
                            f"{lead['share_pct']:.0f}% of units")
                t_k2.metric("EBT / SNAP unit share", f"{ebt_share:.1f}%",
                            help="Share of FunPop units tendered on EBT/SNAP — a price-sensitivity signal.")
                if has_ly:
                    units_ty_tot = float(by_tender["units"].sum())
                    units_ly_tot = float(by_tender["units_ly"].sum())
                    yoy_tot = ((units_ty_tot - units_ly_tot) / units_ly_tot * 100) if units_ly_tot else 0
                    t_k3.metric("Units YoY (all tenders)", f"{int(units_ty_tot):,}",
                                f"{yoy_tot:+.1f}% vs LY")
                else:
                    t_k3.metric("Units per visit (lead)", f"{lead['units_per_visit']:.2f}",
                                help="Average FunPop units per visit on the leading tender.")

                # Units by tender type (current window)
                st.markdown("**Units by tender type (current window)**")
                st.altair_chart((alt.Chart(by_tender.head(12)).mark_bar(color="#2E7D32").encode(
                    x=alt.X("units:Q", title="Units"),
                    y=alt.Y("tender_type:N", sort="-x", title="Tender type"),
                    tooltip=[alt.Tooltip("tender_type:N", title="Tender"),
                             alt.Tooltip("units:Q", format=",", title="Units"),
                             alt.Tooltip("share_pct:Q", format=".1f", title="Unit share %"),
                             alt.Tooltip("units_per_visit:Q", format=".2f", title="Units/visit"),
                             alt.Tooltip("sales:Q", format="$,.0f", title="Sales $")],
                ).properties(height=max(160, 28 * min(12, len(by_tender))))), width='stretch')

                # ── Weekly historical: units by tender over time (TY) ─────────
                st.markdown("**Weekly units by tender type**")
                st.caption(
                    "Units per Walmart week, split by tender. Watch the cash/EBT band swell or "
                    "shrink relative to card — a moving mix is an early read on shopper economics."
                )
                tn_ty["week_start"] = _walmart_week_start(tn_ty["business_date"])
                # Keep the chart legible: only the top tenders get their own band,
                # the long tail rolls into "Other".
                top_types = set(by_tender.head(6)["tender_type"])
                tw = tn_ty.copy()
                tw["tender_label"] = tw["tender_type"].astype("object").fillna("(unspecified)").astype(str)
                tw.loc[~tw["tender_label"].isin(top_types), "tender_label"] = "Other"
                tw_wk = tw.groupby(["week_start", "tender_label"], as_index=False)["units_by_tender"].sum()
                if not tw_wk.empty:
                    tmix_mode = st.radio("Show as", ["Units", "% mix"], index=0, horizontal=True,
                                         key="tender_mix_mode")
                    y_enc = (alt.Y("units_by_tender:Q", stack="normalize", title="Share of units",
                                   axis=alt.Axis(format="%"))
                             if tmix_mode == "% mix"
                             else alt.Y("units_by_tender:Q", stack="zero", title="Units"))
                    st.altair_chart((alt.Chart(tw_wk).mark_area().encode(
                        x=alt.X("week_start:T", title="Week"),
                        y=y_enc,
                        color=alt.Color("tender_label:N", legend=alt.Legend(title="Tender")),
                        tooltip=[alt.Tooltip("week_start:T", title="Week"), "tender_label:N",
                                 alt.Tooltip("units_by_tender:Q", format=",", title="Units")],
                    ).properties(height=300)), width='stretch')

                # ── YoY units by tender (if prior-year data exists) ───────────
                if has_ly:
                    st.markdown("**Year-over-year units by tender type**")
                    st.caption(
                        "Current window vs the same window a year ago (±). A tender shifting faster "
                        "than the total flags where the shopper base is changing."
                    )
                    yoy_long = by_tender.head(10).melt(
                        id_vars=["tender_type"], value_vars=["units", "units_ly"],
                        var_name="Period", value_name="Units")
                    yoy_long["Period"] = yoy_long["Period"].map(
                        {"units": "This Year", "units_ly": "Last Year"})
                    st.altair_chart((alt.Chart(yoy_long).mark_bar().encode(
                        x=alt.X("Units:Q", title="Units"),
                        y=alt.Y("tender_type:N", sort="-x", title="Tender type"),
                        color=alt.Color("Period:N", scale=alt.Scale(
                            domain=["This Year", "Last Year"], range=["#2E7D32", "#A0A09A"])),
                        yOffset="Period:N",
                        tooltip=[alt.Tooltip("tender_type:N", title="Tender"), "Period:N",
                                 alt.Tooltip("Units:Q", format=",", title="Units")],
                    ).properties(height=max(200, 40 * min(10, len(by_tender))))), width='stretch')
                else:
                    st.caption("_Year-over-year tender comparison isn't available in the current feed "
                               "(no prior-year rows returned for this window)._")

                # ── Tender group rollup (units) ───────────────────────────────
                if "tender_group" in tn_ty.columns:
                    grp = tn_ty.groupby("tender_group", as_index=False, observed=True).agg(
                        units=("units_by_tender", "sum"),
                        visits=("visits_by_tender", "sum"),
                    )
                    grp = grp[grp["units"] > 0].sort_values("units", ascending=False)
                    grp["tender_group"] = (grp["tender_group"]
                                           .astype("object").fillna("(unspecified)").astype(str))
                    if len(grp) > 1:
                        st.markdown("**Units by tender group**")
                        st.altair_chart((alt.Chart(grp).mark_bar(color="#1B5E20").encode(
                            x=alt.X("units:Q", title="Units"),
                            y=alt.Y("tender_group:N", sort="-x", title="Tender group"),
                            tooltip=[alt.Tooltip("tender_group:N", title="Group"),
                                     alt.Tooltip("units:Q", format=",", title="Units"),
                                     alt.Tooltip("visits:Q", format=",", title="Visits")],
                        ).properties(height=max(140, 30 * len(grp)))), width='stretch')

    # ── 4. Markdowns — WHERE margin leaks ────────────────────────────────────
    st.divider()
    st.subheader("🏷️ Markdowns & markups — where the price point leaks")
    st.caption(
        "MUMD events on FunPop: dollars given up and units affected when a store cuts (or raises) "
        "the shelf price. Unsanctioned markdowns quietly erode the price point and the brand — "
        "this is the receipt a rep brings to the conversation."
    )
    md_df, md_err = load_mumd_data(lookback, slot)
    if md_err:
        _section_error("Markdown (MUMD) data", md_err)
    elif md_df.empty:
        st.info("No markdown records available.")
    else:
        md_filt = md_df[md_df["walmart_item_number"].isin(item_filter)].copy()
        if md_filt.empty:
            st.info("No markdown activity for the selected items.")
        else:
            md_total = float(md_filt["markdown_amount"].sum())
            md_units = int(md_filt["markdown_units"].sum())
            md_stores = int(md_filt["store_number"].nunique())
            sales_total = float(df["pos_sales_this_year"].sum())
            md_pct = (md_total / sales_total * 100) if sales_total else 0
            m_k1, m_k2, m_k3 = st.columns(3)
            m_k1.metric("Markdown $ (TY)", f"${md_total:,.0f}", help="Total MUMD dollars over the window.")
            m_k2.metric("As % of POS sales", f"{md_pct:.2f}%")
            m_k3.metric("Stores marking down", f"{md_stores:,}",
                        help=f"{md_units:,} units affected over the window.")

            by_event = md_filt.groupby("markdown_event", as_index=False, observed=True).agg(
                amount=("markdown_amount", "sum"),
                units=("markdown_units", "sum"),
            )
            by_event = by_event[by_event["amount"] != 0].sort_values("amount", ascending=False).head(12)
            by_event["markdown_event"] = (by_event["markdown_event"]
                                          .astype("object").fillna("(unspecified)").astype(str))
            if not by_event.empty:
                st.markdown("**Markdown dollars by event type**")
                st.altair_chart((alt.Chart(by_event).mark_bar(color="#791F1F").encode(
                    x=alt.X("amount:Q", title="Markdown $"),
                    y=alt.Y("markdown_event:N", sort="-x", title="Markdown event"),
                    tooltip=[alt.Tooltip("markdown_event:N", title="Event"),
                             alt.Tooltip("amount:Q", format="$,.0f", title="Markdown $"),
                             alt.Tooltip("units:Q", format=",", title="Units")],
                ).properties(height=max(160, 28 * len(by_event)))), width='stretch')

            # Weekly markdown trend
            md_filt["week_start"] = _walmart_week_start(md_filt["markdown_date"])
            md_wk = md_filt.groupby("week_start", as_index=False)["markdown_amount"].sum().sort_values("week_start")
            md_wk["week_label"] = md_wk["week_start"].dt.strftime("Wk %b %d")
            if len(md_wk) > 1:
                st.markdown("**Markdown dollars by week**")
                st.altair_chart((alt.Chart(md_wk).mark_bar(color="#A8443B").encode(
                    x=alt.X("week_label:N", sort=list(md_wk["week_label"]), title="Week",
                            axis=alt.Axis(labelAngle=-30)),
                    y=alt.Y("markdown_amount:Q", title="Markdown $"),
                    tooltip=["week_label", alt.Tooltip("markdown_amount:Q", format="$,.0f", title="Markdown $")],
                ).properties(height=240)), width='stretch')

    # ── 5. Out-of-stock root cause — WHY we go dark ──────────────────────────
    st.divider()
    st.subheader("📦 Out-of-stock root cause — why we go dark")
    st.caption(
        "The reason hierarchy behind each store-day FunPop is unavailable. Turns a vague "
        "'we're out' into a specific, fixable cause (allocation, DC dwell, store DRP, forecast) "
        "a rep can escalate to the right owner."
    )
    oos_df, oos_err = load_oos_data(lookback, slot)
    if oos_err:
        _section_error("Out-of-stock root-cause data", oos_err)
    elif oos_df.empty:
        st.info("No out-of-stock root-cause records available.")
    else:
        oos_filt = oos_df[oos_df["walmart_item_number"].isin(item_filter)].copy()
        if oos_filt.empty:
            st.info("No out-of-stock root-cause data for the selected items.")
        else:
            o_k1, o_k2, o_k3 = st.columns(3)
            o_k1.metric("Store-days flagged OOS", f"{len(oos_filt):,}")
            o_k2.metric("Avg fractional in-stock",
                        f"{oos_filt['fractional_instock_pct'].mean():.1f}%",
                        help="Average fractional in-stock % across flagged store-days "
                             "(lower = more of the day spent out of stock).")
            o_k3.metric("Stores affected", f"{oos_filt['store_number'].nunique():,}")

            # Group by the most actionable (low-level) reason; fall back to high-level.
            reason_col = "oos_reason_low"
            tmp = oos_filt.copy()
            tmp[reason_col] = tmp[reason_col].astype("object").fillna("").astype(str).str.strip()
            tmp.loc[tmp[reason_col] == "", reason_col] = (
                tmp["oos_reason_high"].astype("object").fillna("(unknown)").astype(str))
            by_reason = tmp.groupby(reason_col, as_index=False).agg(
                store_days=("store_number", "size"),
                stores=("store_number", "nunique"),
            )
            by_reason = by_reason[by_reason[reason_col].str.strip() != ""].sort_values(
                "store_days", ascending=False).head(15)
            by_reason = by_reason.rename(columns={reason_col: "reason"})
            if not by_reason.empty:
                st.markdown("**Top out-of-stock reasons (by store-days)**")
                st.altair_chart((alt.Chart(by_reason).mark_bar(color="#B26A00").encode(
                    x=alt.X("store_days:Q", title="Store-days flagged"),
                    y=alt.Y("reason:N", sort="-x", title="Root cause"),
                    tooltip=[alt.Tooltip("reason:N", title="Root cause"),
                             alt.Tooltip("store_days:Q", format=",", title="Store-days"),
                             alt.Tooltip("stores:Q", format=",", title="Stores")],
                ).properties(height=max(180, 26 * len(by_reason)))), width='stretch')
                lead = by_reason.iloc[0]
                st.caption(
                    f"**Biggest lever:** *{lead['reason']}* drives {int(lead['store_days']):,} of the "
                    "flagged store-days — the single root cause worth escalating first."
                )
